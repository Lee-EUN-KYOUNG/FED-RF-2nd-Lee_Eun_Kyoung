...
// 참고) JS코드만 실행 확장프로그램: 
// Code Runner(+ Node JS설치후 리부팅) - Node JS 프로그램 설치 필수!
...
console.log('할당후 최초상태: \nkk', kk , '\ntt:',tt);
// 일반적인 값은 값 복사가 맞다!
// 그러나 배열/객체일 경우 컬렉션 집합을 만들고 주소로 관리가 되기 때문에
// 변수에 변수를 할당할때는 값이 아닌 주소가 복사된다!
...
// 스프레드 연산자 (...) 사용 -> [...배열변수]
// 스프레드 연산자는 배열의 값만 가져옴

let dd = [...cc];

console.log('dd 변경 전 값: \ncc', cc , '\ndd:',dd);

// 스프레드 연산자로 값만 복사후 새배열 특정값 변경
dd[0] = 888;
...
/* let hh = [...gg]; -> 러너 검증 결과 효과없음 */

// 깊은복사방법은 기존 객체 배열값을 문자화하고 다시 객체화한다!
// -> JSON.parse(JSON.stringify(배열값))
// parse() 매서드는 구문을 분석하여 원래 구문 형식으로 변환해주는 역할을 한다
// stringfy() 매서드는 어떤 값이든 문자화하여 변환해줌!

// 원리: 문자화하는 순간 배열이 값으로 변경되어 새로운 주소지가 된다!
// 다시 이값을 배열로 복원하면 깊은복사 완성!!!
// 객체값을 가지는 배열로 검색/정렬할때 복사는 깊은복사를 해야 원본손상이 없다!!!!
