...
function showTime(){
    ...
    // 월과 요일은 배열 순번으로 리턴된다 -> 나라마다 월과 요일을 부르는 이름이 다르기 때문
    // 월을 숫자로 출력하려면 1만 더하면 된다
   ...
   //S = 5;
    ...
    // 24제로 값이 넘어오기때문에 12를 빼준다
    H = H>12?H-12:H;

    // 0붙이기 리턴함수
    const addZero = x => x<10?"0" + x : x;

    // 시,분,초에서 한자리 숫자는 앞에 0자리수 넣기
...
// 배열만큼 이미지 넣기
    ...
    //    imbx.innerHTML += `<img src="${rimg[i]}" class="on">`;
    //}
    ...
    function randomAddOn(){
        ...
        // 3. while문 통과후 결정된 난수를 직전 난수 변수에 할당
        beNum = rdm;

        // 4. 랜덤으로 발생한 난수에 해당하는 div에 on 클래스를 추가한다
