...
-- 세번째 레코드 입력
INSERT INTO `friends` (`fname`,`ftel`,`faddr`,`fmsg`) 
VALUES ("강감찬","01011114444","서울시 관악구","나라에 충성~!!!");

-- 선택 쿼리
SELECT fname FROM `friends` WHERE fname = "김종서";
SELECT `faddr` FROM `friends` WHERE `faddr` like '%관악구';

-- 선택항목 별칭 (as)으로 출력하기
SELECT `fname` as "이름",`faddr` as "주소",`ftel` as "전화번호" FROM `friends` WHERE `faddr` like '%관악구';