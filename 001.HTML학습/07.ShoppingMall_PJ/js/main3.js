...
function loadFn() {
  ...
  // (3) 이때 한계값을 체크하여 순환되게 함 -> 끝번호 뒤는 첫번호, 첫번호 앞은 끝번호로 순환되게 함
  // (4) 해당 순번의 슬라이드에 class.on 넣기 -> 나머지 슬라이드는 on 제거하기 (외부 함수로 구성)
  // (5) 블릿표시자도 슬라이드와 같은 순번에 class on 넣고 나머지는 빼준다 (외부 함수로 구성)
  // (6) 자동 넘김 구성 함수를 호출하여 인터발 호출 작동 -> 버튼 클릭시 인터발 지우기/일정시간 뒤 다시 인터발 작동
