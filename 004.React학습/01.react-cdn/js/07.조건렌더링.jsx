...
function MakeImage({ isrc, ialt, itit, icss }) {
  ...
  // 속성 중 안 보낸것은 출력되지 않는다
} ////////////////// MakeImage 컴포넌트 /////////////

///////////////////////////////////////// 메인 출력 컴포넌트 ////////////////////////////////////
// 컴포넌트 내부에서 호출하는 다른 컴포넌트를 보통 '서브 컴포넌트'라고 부른다
function Developer({ isNormal, isrc, ialt, itit }) {
  ...
  // 위 if문 return에 걸리면 아랫쪽 return에는 내려오지 않는다
...
/********************************************** 
    ...
    -> 조건식은 true / false의 결과가 나오는 식이며
     AND 조건 , OR 조건, NOT 조건식이 모두 사용 가능함
     예) A == "사자" && B > 10 || C ="별" || !D
**********************************************/
...
function MakeList({ foodName, movieInfo }) {
  ...
  return (
    <li>
      {
        // 음식 데이터가 들어온 경우 undefined가 아니면 true이므로  && 뒤 코드 출력
...
function WishList({ wList }) {
  ...
  return (
    <React.Fragment>
      ...
      {wList.length > 0 && (
        <div>
          <h2>개발자가 좋아하는 음식 모두 {wList.length}가지입니다</h2>
        </div>
      )}
      <ul>
        {
          ...
          // 그러나 리액트는 이것을 변경하여 표현식안에서 출력시 태그 JSX 형식으로 변화해줌
...
// 구성 : 작가 이름 + 작품 이미지
// 데이터 : 작가 이름 (painter) 이미지 경로 (작가 이름의 객체 workUrl) 작품명 (wname)
...
// 4-2. 전체 출력 컴포넌트 ///////////
// 구성 : 전체 타이틀(SetTitle 컴포넌트) + 변경 버튼
//        + 작가 타이틀과 그림 출력 (MakePainting 컴포넌트)
// 특이사항 : 변경 버튼 클릭시 MakePainting 컴포넌트의 데이터를 변경하여 다시 출력하도록 함 (Hook 사용)

function ShowLikePinter({ isChange }) {
  // isChange - 기존 작가를 변경하는 여부 전달 변수
  // true / false -> true면 바꿈
