...
export default function 이야기() {
  // 컨텍스트 프로바이더 불러오기 - useContext() 메서드 사용!
  ...
  // 기존 산 정보 객체의 배열에서 산 이름값만 모아서 새로운 배열을 만든다!
  // 이것이 map 메서드의 본래 사용법
  const mtTotal = mtInfo.map((v) => v.이름);
  // console.log("산이름만 새 배열:", mtTotal);

  // 산 이름 변경 메서드
  const changeMtName = (e) => {
    console.log(e.target.innerText);
    // 부모 프로바이더로부터 전달받은 상태변수 변경 메서드를 사용하여 산 이름을 업데이트한다
...
<h1>
      ...
      {mtTotal.map(
        (v) =>
          v != 나야나.mtName && (
            <button
              style={{ padding: "15px", fontSize: "20px", margin: "10px" }}
