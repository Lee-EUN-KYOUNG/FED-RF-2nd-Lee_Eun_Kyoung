...
function App() {
  ...
  //// 국적 표시 객체 -> 매번 컴포넌트가 리랜더링 될떄마다 할당되는 객체의 주소값이 변경된다
  /*  const nara = {
    country: isKor ? "한국" : "일본",
  }; */

  // 처음 할당된 객체의 주소값을 메모하여 재사용하면
  // 다른 상태 변수가 변경될때 같은 주소값을 유지하여
  // nara 변수가 변경되지 않도록 한다
  // 사용법 : 변수 = React.useMemo(()=>{return객체})
...
/// 제목을 뿌리는 서브 컴포넌트
// -> 초기 셋팅 후 변경 내용이 없는 서브 컴포넌트가 리랜더링 되지 않도록 하는 방법은?
// --> React.memo(()=>{})
// 주의 사항 : 컴포넌트가 할당형으로 만들어져야함
const ShowTit = React.memo(({txt}) =>{

    console.log("ShowTit 컴포넌트 랜더링");
    return <h1 style={{fontSize:"50px"}}>{txt}</h1>;
