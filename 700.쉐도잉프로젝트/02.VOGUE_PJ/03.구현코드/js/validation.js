...
form.logF input[type=password]`;
  ...
  $(tgInput).blur(function () {
    ...
    ******************************************/
    let cv = $(this).val();
    // cv - current value, 현재값
    // val() - 입력값 읽어오기

    // trim() - 앞뒤 공백 제거 -> 중간 공백 제거 안함
    // 모든 공백을 제거하는 함수를 만들어 쓴다
    const groSpace = (x) => x.replace(/\s/g, "");

    // get rid of space - 공백 제거 함수
    // 정규식은 슬래쉬(/) 사이에 쓰며 역슬래쉬(\s)는 스페이스 기호이고 g는 전역(global) 플래그
    // 플래그란 기술 용어로 처리 표시 기호를 뜻함

    // 이름(#mnm)일 경우 중간 공백 있어야 하므로 삼항연산자로 trim을 함께 사용한다
  ...
  ********************************************/
  ...
  seleml.change(function () {
    ...
    if (cv == "init") {
      ...
      // 직접 입력창 숨기기 - fadeOut(시간,이징,함수),페이드아웃 효과 메서드(제이쿼리)
      eml2.fadeOut(300);
    } /////// if : 선택해주세요 ///////

    // 2-2.'직접입력'일 경우
    else if (cv == "free") {
      // 입력창 보이기 - fadeIn(시간,이징,함수),페이드인 효과 메서드(제이쿼리)
      // + 값 지우기 + 포커스 추가하여 직접 입력창 텍스트 초기화
      eml2.fadeIn(300).val("").focus();

      // 이메일 옵션 선택 필수 메시지 지우기
 ...
 ***********************************************/
  ...
  $("#email1, #email2").on("keyup", function () {
    // 현재 이벤트 발생 대상 아이디 읽어오기
    let cid = $(this).attr("id");

    //console.log("입력창id:",cid);

    // 이메일 뒷주소 셋팅하기
    // -> 현재 입력한 아이디가 "email2"이면 직접 입력창을 읽고
    // 아니면 선택 박스 값이 "free"가 아닌 경우 선택 박스값 읽고
    // 아니면 직접 입력창값을 뒷주소로 설정함
  ...
  ***********************************************************************************************/
  ...
  /*************************************************************************************************** 
    ...
    ***************************************************************************************************/
  $(".eye")
    .css({ textDecoraion: "line-through", opacity: 0.5, cursor: "pointer" })
    .click((e) => {
      // 투명도 값 읽기
      let opa = $(e.target).css("opacity");
      console.log(opa);

      // 글자 보이기 전환 - 투명도가 0.5일때 보이게함
      $("#mpw").attr("type", opa == "0.5" ? "text" : "password");

      // CSS 디자인 전환
      $(e.target).css({
        textDecoraion: opa == "0.5" ? "none" : "line-through",
  ...
  *********************************************/
    ...
    $("#btnj").click((e)=>{
      ...
      // pass 통과여부 변수에 true 할당하기
      pass = true;

      // 입력창 blur 이벤트 강제 발생하기
