...
// 생성자 함수로 만들어서 사용하기
/// -> 컴포넌트별로 변수와 함수가 별도로 메모리 구역이 설정됨
// -> 변수,함수의 지역화가 가능해짐! 같은 이름의 변수,함수로 서로 상관될 일이 없어짐
// ((생성자함수의 필수요건))
// 1. 생성자 함수는 대문자로 시작하는 이름을 가짐
// 2. 생성자 함수는 호출시 new 키워드로 생성함
...
function SlideFn() {
  ...
  // this.변수 = 화살표 함수
  // -> 함수인 변수를 this 키워드로 등록하면 생성자 함수를 담은 변수에서 하위 변수로 노출된다! 그래서 사용 가능
